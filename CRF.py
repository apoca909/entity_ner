...
class CRFModule( nn.Module):
    ...
    emission score tensor. This class also has `~torch_crf.decode` method which finds
    ...
    def forward(self, emissions: torch.Tensor,
        ...
        """
        #self._validate(emissions, mask=mask, tags=None)

        if mask is None:
            mask  = torch.ones(list(emissions.shape)[:2], dtype=torch.uint8)
